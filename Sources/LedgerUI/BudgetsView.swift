                                ...
                                VStack(alignment: .leading, spacing: 14) {
                                    HStack(spacing: 14) {
                                        ...
                                        Text(MoneyFormatter.string(from: -progress.spentMinor))
                                            ...
                                            .foregroundStyle(progress.remainingMinor < 0 ? LedgerTheme.negative : LedgerTheme.textPrimary)
                                    }
                                    ...
                                    Text(progress.remainingMinor < 0 ? "Over budget by \(MoneyFormatter.string(from: abs(progress.remainingMinor)))" : "Remaining \(MoneyFormatter.string(from: progress.remainingMinor))")
                                        .font(.system(size: 14, weight: .medium))
                                        .foregroundStyle(progress.remainingMinor < 0 ? LedgerTheme.negative : LedgerTheme.textSecondary)
                                }
                                .padding(20)
            ...
            HStack(spacing: 10) {
                ForEach(monthOptions, id: \.self) { monthKey in
                    Button {
                        ...
                        Text(LedgerTheme.monthAbbreviation(for: monthKey))
                            .font(.system(size: 14, weight: .bold))
                            .foregroundStyle(model.selectedMonthKey == monthKey ? .white : LedgerTheme.textSecondary)
                            .frame(width: 52, height: 40)
                            .background(
                                Capsule()
                                    .fill(model.selectedMonthKey == monthKey ? LedgerTheme.textPrimary : LedgerTheme.surface)
                            )
                            .overlay(Capsule().stroke(model.selectedMonthKey == monthKey ? .clear : LedgerTheme.line, lineWidth: 1))
                    }
                }
            }
        }
                        ...
                        if instance.status == .scheduled {
                            HStack(spacing: 10) {
                                Button("Post") { model.postInstance(instance) }
                                    ...
                                    .padding(.vertical, 8)
                                    .background(LedgerTheme.accentDark, in: Capsule())
