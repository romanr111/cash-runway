...
struct TransactionEditorView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .top) {
                LedgerTheme.background.ignoresSafeArea()
    ...
    private var composerHeader: some View {
        ...
        .padding(.top, 16)
        .padding(.bottom, 28)
        .background(LedgerTheme.composerHeader)
            ...
            VStack(spacing: 0) {
                ...
                Button {
                    ...
                    Text("Save Transaction")
                        ...
                        .padding(.vertical, 18)
                        .background(LedgerTheme.accentDark, in: RoundedRectangle(cornerRadius: 24, style: .continuous))
                }
                .padding(.top, 20)
                .padding(.bottom, 32)
            }
    ...
    private var divider: some View {
        Divider().overlay(LedgerTheme.line)
    }
...
struct CategoryManagementView: View {
    ...
    @State private var categoryDraft = LedgerCategory(id: UUID(), name: "", kind: .expense, iconName: "questionmark.app.fill", colorHex: "#7E57C2", parentID: nil, isSystem: false, isArchived: false, sortOrder: 0, createdAt: .now, updatedAt: .now)

    init(model: LedgerAppModel, initialKind: CategoryKind) {
        self.model = model
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                .pickerStyle(.segmented)
                .listRowBackground(LedgerTheme.background)
                    ...
                    HStack(spacing: 14) {
                        ...
                        Button {
                            ...
                            Image(systemName: item.isVisible ? "eye.fill" : "eye.slash.fill")
                                .font(.system(size: 17, weight: .semibold))
                                .foregroundStyle(item.isVisible ? LedgerTheme.accentDark : LedgerTheme.textMuted)
...
struct CategoryEditorView: View {
    @Environment(\.dismiss) private var dismiss
    @Bindable var model: LedgerAppModel
    @Binding var category: LedgerCategory
