            ...
            HStack(spacing: 10) {
                ForEach(model.overviewSnapshot?.months ?? []) { point in
                    Button {
                        ...
                        Text(CashRunwayTheme.monthAbbreviation(for: point.monthKey))
                            .font(.system(size: 14, weight: .bold))
                            .foregroundStyle(point.monthKey == model.selectedMonthKey ? .white : CashRunwayTheme.textSecondary)
                            .frame(width: 52, height: 40)
                            .background(
                                Capsule()
                                    .fill(point.monthKey == model.selectedMonthKey ? CashRunwayTheme.textPrimary : CashRunwayTheme.surface)
                            )
                            .overlay(
                                Capsule()
                                    .stroke(point.monthKey == model.selectedMonthKey ? .clear : CashRunwayTheme.line, lineWidth: 1)
                            )
                    }
                }
            }
