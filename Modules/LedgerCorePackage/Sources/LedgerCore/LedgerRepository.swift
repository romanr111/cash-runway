...
public final class LedgerRepository: @unchecked Sendable {
    ...
    public func saveBudget(_ budget: Budget) throws {
        guard budget.limitMinor > 0 else {
            throw LedgerError.validation("Budget limit must be greater than zero.")
        }
        ...
        try databaseManager.dbQueue.read { db in
            ...
            if transaction.type == .transferOut {
                guard let linkedID = transaction.linkedTransferID,
                    ...
                    throw LedgerError.invalidState("Transfer pair is missing.")
                }
    ...
    private func saveSingleTransaction(_ db: Database, draft: TransactionDraft, updateDerivedData: Bool = true) throws {
        ...
        let existing: LedgerTransaction? = if let draftID = draft.id {
            try existingTransaction(db, id: draftID)
        } else {
            nil
        }
        let ledgerType: TransactionKind = draft.kind == .expense ? .expense : .income
    ...
    private func saveTransfer(_ db: Database, draft: TransactionDraft, updateDerivedData: Bool = true) throws {
        guard let destinationWalletID = draft.destinationWalletID, destinationWalletID != draft.walletID else {
            throw LedgerError.validation("Transfer requires two different wallets.")
        }
        let now = Date()
        let sourceID = draft.id ?? UUID()
        let sourceExisting: LedgerTransaction? = if let draftID = draft.id {
            try existingTransaction(db, id: draftID)
        } else {
            nil
        }
        let targetExisting: LedgerTransaction? = if let linkedTransferID = sourceExisting?.linkedTransferID {
        ...
        let sourceRecord = LedgerTransaction(
            id: sourceID,
            walletID: draft.walletID,
            type: .transferOut,
        ...
        let targetRecord = LedgerTransaction(
            id: targetID,
    ...
    private func existingTransaction(_ db: Database, id: UUID) throws -> LedgerTransaction? {
        guard let row = try Row.fetchOne(db, sql: "SELECT * FROM transactions WHERE id = ?", arguments: [id.uuidString]) else {
            return nil
        }
    ...
    private func upsertTransactionRow(_ db: Database, transaction: LedgerTransaction) throws {
        try db.execute(
            sql: """
    ...
    private func validate(_ draft: TransactionDraft) throws {
        guard draft.amountMinor > 0 else {
            throw LedgerError.validation("Amount must be greater than zero.")
        }
        if draft.kind != .transfer, draft.categoryID == nil {
            throw LedgerError.validation("Category is required for income and expense transactions.")
        }
    }
    ...
    private func contribution(for transaction: LedgerTransaction) -> AggregateContribution {
    ...
    private func syncSearch(_ db: Database, transaction: LedgerTransaction) throws {
    ...
    private static func transaction(_ row: Row) throws -> LedgerTransaction {
        LedgerTransaction(
            id: UUID(uuidString: row["id"])!,
